...
> **备注：**`attr()` 函数可以和任意 CSS 属性使用，但对于 [`content`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/content) 之外的属性的支持都是实验性的。

`attr()` [CSS](https://developer.mozilla.org/zh-CN/docs/Web/CSS) [函数](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Values/Functions)用于检索所选元素的属性值并应用于属性值，类似 [`var()`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Values/var) 替换自定义属性值的方式。可以在[伪元素](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-elements)上使用，在这种情况下，将返回伪元素的源元素上的属性值。
...
CSS `animation-timeline` 是一种新特性，用于将动画的进度与页面事件（如滚动等时间线）进行关联。它可以让动画不再仅依赖于时间推进，而是与其他页面交互行为绑定，实现如滚动驱动的动画效果等。

以下是应用示例：
:::code-group
```html
<div class="block">Your content</div>
```
...
```css
.zoom-container {
  contain: layout style paint;  /* 容器隔离：SVG内部重绘不触发页面其他区域 layout */
}

.station-map-container {
  will-change: transform;  /* 合成层提升：拖拽/缩放只走 GPU compositor，不触发 repaint，但可能导致矢量图失真 */ 
}
```
...
.tool-item:hover {
  ...
  box-shadow: 0 12px 19px -4px rgba(0, 0, 0, 0.118),
    0px 4px 5px -3px rgba(0, 0, 0, 0.09);
}
</style>
